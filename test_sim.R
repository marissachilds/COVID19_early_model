...
## Parameters needed to set up parallel
num_cores     <- 2
registerDoParallel(cores = num_cores)
loglik.thresh <- 2

## load and check number of rows above the threshold for splitting up
rds.name   <- "output/Santa_Clara_TRUE_FALSE_2020-04-01_2021-02-08_final.Rds"
traj.name  <- "output/Santa_Clara_TRUE_FALSE_2020-04-01_2021-02-08_final_filter_traj.rds"
prev.fit   <- readRDS(rds.name)
variable_params   <- prev.fit[["variable_params"]] %>% mutate(rowid = 1:n())
variable_params   <- variable_params %>% 
  filter(log_lik != 0) %>% 
  filter(log_lik >= (max(log_lik) - loglik.thresh)) %>% arrange(desc(log_lik)) 

## Split up 
var.seq   <- seq(1, nrow(variable_params))
sim_split <- split(var.seq, ceiling(seq_along(var.seq)/(length(var.seq)/num_cores)))

system.time(
fit.out   <- foreach(num_batch = seq_along(sim_split), .combine = list, .multicombine = TRUE) %dopar%  {
...
, ntraj          = 5
, traj.trim_date = NA
, which_params   = sim_split[[num_batch]]
)
  
}
)

COVID_simulate_adj <- function(rds.name, 
...
variable_params <- variable_params %>% 
  ...
  mutate(orderedid = 1:n()) %>%
  filter(orderedid %in% which_params)
